...
std::string read_file_into_string(std::string_view filename)
{
    ...
    if (!file.is_open())
    {
        throw std::runtime_error("Could not open file: " + std::string(filename) + " - " + std::strerror(errno));
    }
...
int benchmark(const std::string &str, const std::string &substr, size_t seconds_to_run, std::function<int(std::string_view, std::string_view)> func)
{
...
int benchmark_simd_vs_std(const std::string &str, const std::string &substr, size_t seconds_to_run)
{
    auto result_simd = benchmark(str, substr, seconds_to_run, simd_strstr);
    auto result_std = benchmark(str, substr, seconds_to_run, [](std::string_view str, std::string_view substr)
              { return str.find(substr); });
    if(result_simd != result_std)
    {
        std::cout << "Mismatch: " << result_simd << " vs " << result_std << std::endl;
    }
    return result_simd;
}
